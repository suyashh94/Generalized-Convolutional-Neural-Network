...
function [Z,A,Z_bn_unfolded,Z_hat_unfolded,gamma,...
            Z_mu_unfolded, ivar, var_sqrt, var, eps,max_mask] = Conv_forward_prop_dropout_BN(X,conv_layer_name,filt,s_x,s_y,s,toKeep_matrix,keep_prob,gamma,beta)
...
for l = 1:num_conv_layers
    ...
    Forward_Convolution_And_Relu_BNN_dropout(A_prev,filt{l},s_x{l},s_y{l},toKeep_matrix{l},keep_prob,gamma{l},beta{l});
    ...
    elseif conv_layer_name(l) == 2
        ...
        [A{l},Z_bn_unfolded{l},Z_hat_unfolded{l},gamma{l},Z_mu_unfolded{l}, ivar{l}, var_sqrt{l}, var{l}, eps{l}] ...
            = batch_norm_forward_maxpool(Z{l},gamma{l},beta{l});
        if l < length(conv_layer_name)
            max_mask{l} = max_pool_masks(A{l},s{l+1});
        else
            max_mask{l} = max_pool_masks(A{l},1);
